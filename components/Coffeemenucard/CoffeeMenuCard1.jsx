...
export default function CoffeeCard1() {
  ...
  const ringLabel = useMemo(
    () => "SPECIALTY ROASTERS RESERVE • CRAFTED EST. 2020 • ",
    [],
  );
  ...
  return (
    <section className="coming-section" ref={sectionRef}>
      {/* Background Atmosphere */}
      <div className="coming-noise" aria-hidden="true" />
      <div
        className="coming-ambient-glow"
        style={{
      ...
      <header className="coming-header">
        <div className="eyebrow-container">
          ...
          <span className="coming-eyebrow">The Reserve Collection</span>
          <Sparkles className="eyebrow-sparkle" />
        </div>

        <h1 className="coming-title">Artisanal Coffee Selector</h1>
        <p className="coming-subtitle">
          Select from our master roasters single-origin harvests & precision
          pulls
        </p>

        <div className="coming-rule">
          <span className="rule-diamond" />
        </div>

        {/* Category Tabs */}
        <nav className="coming-tabs" aria-label="Coffee categories">
          {categories.map((cat) => {
